...
vec3 ApplyLight(Light light, vec3 vertexN, vec3 viewN, vec3 lightPos) {
    ...
    //ambient
    vec3 ambient = light.ambientCoefficient * col * light.intensities;
    
    //diffuse
    vec3 diffuse = (col/2.0) * max(dot(vertexN, lightN), 0) * light.intensities;
    
    //specular
    float alpha = matShine;
    vec3 halfValue = normalize(viewN + lightN);
    vec3 specular = (col/2.0) * pow(max(dot(vertexN, halfValue), 0), alpha) * light.intensities;
