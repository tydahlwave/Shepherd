...
void CameraController::Update(World &world) {
    ...
    glm::vec3 gaze = camera->lookAt - world.mainCharacter->transform->position;
    glm::vec3 w = normalize(-gaze);
    glm::vec3 u = normalize(cross(camera->up, w));
    if (abs(rigidBody->velocity[2]) > cameraStoppedThreshold) {
        world.mainCharacter->transform->position += rigidBody->velocity[2] * w;
        camera->lookAt += rigidBody->velocity[2] * w;
    }
    if (abs(rigidBody->velocity[0]) > cameraStoppedThreshold) {
        if ((world.mainCharacter->transform->position.y + rigidBody->velocity[0] * u.y) > -1.99) {
            u.y = 0;
    ...
    if (world.mainCharacter->transform->position.y < -1.99) {
        world.mainCharacter->transform->position.y = -1.99;
    }
}
...
void CameraController::MouseMoved(World *world, int windowWidth, int windowHeight, double mouseX, double mouseY) {
    ...
    Camera *camera = (Camera*)world->mainCharacter->GetComponent("Camera");
	camera->lookAt[0] = world->mainCharacter->transform->position[0] + camera->offset[0] + cos(alpha2) * cos(beta2);
    camera->lookAt[1] = world->mainCharacter->transform->position[1] + -sin(alpha2);
    camera->lookAt[2] = world->mainCharacter->transform->position[2] + camera->offset[2] + cos(alpha2) * cos(M_PI/2 - beta2);
