...
CAudioEngine* CAudioEngine::instance()
{
    if (!a_instance)
        a_instance = new CAudioEngine;
    return a_instance;
}


//Loads current sounds in lib
void CAudioEngine::LoadSounds(std::string resDir)
{
    CAudioEngine::resourceDir = resDir;
    
    instance()->Loadsound((resDir + "baa.wav"), true, false, false);
    instance()->Loadsound((resDir + "back.wav"), false, true, false);
    instance()->Loadsound((resDir + "fun.wav"), false, true, false);
...
int CAudioEngine::PlaySound(const string& strSoundName, const glm::vec3& vPosition, float fVolumedB)
{
    ...
    auto tFoundIt = sgpImplementation->mSounds.find(location);
    if (tFoundIt == sgpImplementation->mSounds.end())
    {
        Loadsound(location);
        tFoundIt = sgpImplementation->mSounds.find(location);
        if (tFoundIt == sgpImplementation->mSounds.end())
        {
            return nChannelId;
        }
